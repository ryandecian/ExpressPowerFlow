...
function statusAC_dataOn_ZSF2400AC_N1_Service(dataZendure) {
    let status = false;
    ...
    if (dataZendure.properties.gridState === 0) {
        ...
        if (getMemory_Memory().zendureSolarflow2400AC_N1.data.properties.gridState === 0) {
            status = false;
        }
        else if (getMemory_Memory().zendureSolarflow2400AC_N1.data.properties.gridState === 1) {
            console.warn("zendureSolarflow2400ACN1_Controller => statusAC_dataOn_ZSF2400AC_N1_Service : Le courant AC n'est plus détecté par la batterie Zendure Solarflow 2400 AC N1.");
            status = false;
        }
        else {
    ...
    /* Si le courant AC est détecté */
        ...
        /* On vérifie le status précédent avant de loger */
        if (getMemory_Memory().zendureSolarflow2400AC_N1.data.properties.gridState === 1) {
            status = true;
        }
        else if (getMemory_Memory().zendureSolarflow2400AC_N1.data.properties.gridState === 0) {
