...
function statusAC_dataOff_ZSF2400AC_N2_Service(dataZendure: GetZendureSolarflow2400AC_data_Type): boolean {
    let status = false;
    ...
    /* Logique métier 1 : La batterie détecte il le courant AC ? */
        ...
        /* Si le courant AC n'est pas détecté */
                ...
                console.warn("zendureSolarflow2400ACN2_Controller => statusAC_dataOff_ZSF2400AC_N2_Service : Le courant AC n'est pas détecté par la batterie Zendure Solarflow 2400 AC N2.");
                status = false;
            }
        /* Si la valeur n'est pas valable, on ne sait pas si le courant est détectée, on mise sur la sécurité */
            else {
                console.error("zendureSolarflow2400ACN2_Controller => statusAC_dataOff_ZSF2400AC_N2_Service : Mise en sécurité : Le server n'arrive pas à déterminer si la batterie Zendure Solarflow 2400 AC N2 détecte le courant AC.");
                status = false;
            }
    /* Logique métier 2 : La batterie est elle synchronisée au courant AC ? */
        ...
        /* Si la batterie détecte le courant AC et est en cours de synchronisation */
                ...
                console.info("zendureSolarflow2400ACN2_Controller => statusAC_dataOff_ZSF2400AC_N2_Service : La batterie Zendure Solarflow 2400 AC N2 détecte le courant AC et est en cours de synchronisation.");
                status = false;
            }
        /* Si la batterie ne détecte pas le courant AC */
            else if (dataZendure.properties.acStatus === 0) {
                console.warn("zendureSolarflow2400ACN2_Controller => statusAC_dataOff_ZSF2400AC_N2_Service : La batterie Zendure Solarflow 2400 AC N2 n'est plus synchronisée au courant AC.");
                status = false;
            }
        /* Si la valeur n'est pas valable, on ne sait pas si la batterie est synchronisée, on mise sur la sécurité */
            else {
                console.error("zendureSolarflow2400ACN2_Controller => statusAC_dataOff_ZSF2400AC_N2_Service : Mise en sécurité : Le server n'arrive pas à déterminer si la batterie Zendure Solarflow 2400 AC N2 est synchronisée au courant AC.");
                status = false;
            }
...
export { statusAC_dataOff_ZSF2400AC_N2_Service };
