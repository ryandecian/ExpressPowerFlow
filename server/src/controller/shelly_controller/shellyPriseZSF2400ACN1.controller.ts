    ...
    try {
        ...
        /* Logique métier 1 : Récupération des données du compteur */
            ...
            if (typeof dataShellyResult.error === "string") {
                console.error("shellyPriseZSF2400ACN1_Controller - Erreur de fetch :", dataShellyResult.error);

                /* Si les données ont déjà été initialisées en mémoire */
                    if (getMemory_Memory().shellyPrise_BatterieZSF2400AC_N1 !== null) {
                        setMemory_Lvl2_Memory("shellyPrise_BatterieZSF2400AC_N1", "status", false);
                    }
                return;
            }
        ...
        /* Logique métier 2 : Préparation des données pour l'enregistrement */
            const dataSelected: ShellyPlugSGen3_Snapshot_Type = {
                ts: Date.now(),
                source: "Prise Shelly ZSF2400AC N1",
                status: true,
                data: {
        ...
        /* Logique métier 4 : Récupération des données depuis la mémoire pour vérification */
            ...
            console.log(`Compteur Shelly Plug S Gen 3 de la batterie Zendure Solarflow 2400AC N1 : ${dataSelected.data.apower} W`);
        const end = Date.now();
